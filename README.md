...
## 🎯 Business Intent（ビジネス意図と提供価値）

### 【コアバリュー】人間関係の「事故」防止と、社会的生活のハック

現代社会において、良好な人間関係の維持には多大な「認知負荷（記憶・気遣い・判断）」が伴います。本プロジェクトは、この重労働の動機を「相手への思いやり」から**「致命的な社会的事故の回避（保身・防衛）」**へと完全に再定義した、究極の保身・防衛ツールです。
これまで「不確実な善意」や「能動的な記憶力」に依存していた関係維持のプロセスを、AIと自動化パイプラインによってシステムへ強制置換し、ユーザーを思考停止状態のまま社会的勝者へと導きます。

### 【システムが生み出す皮肉（Systematic Irony）】怠慢と評価の反比例

一切の努力を放棄してサボっているにもかかわらず、周囲からの評価が向上し、より「生きやすくなる」。この皮肉な逆転現象こそが、本サービスの真の価値です。

* **ユーザーの内面（Input）**: 相手への関心ゼロ、記憶ゼロ、判断ゼロ。人間関係の維持を徹底的に「サボる」。
* **社会からの評価（Output）**: 「マメな人」「気遣いができる完璧な人」として評価が右肩上がりに上昇。

「心を込めない（AIに丸投げする）方が、人間関係のシステムはバグらずに完璧に稼働する」という、感情や忘却といったヒューマンエラーを排除した結果生じるこの皮肉こそが、ユーザーを本サービスに深く依存させる強力なフックとなります。

### 【技術的ハイライト】「思考停止」を支える3つの柱

ユーザーが一切の「迷い」や「葛藤」を抱かずにシステムに身を委ねるため、以下の3点を技術的に担保しています。

1.  **「圧ゼロ」思考停止型UI（UI/UX）**
    * 選択肢の徹底排除。AIが算出した単一の「正解（カンペ/ギフト）」のみを提示します。
    * スワイプと1-Clickに特化し、アプリ内での滞在時間（＝思考時間）を物理的に最小化します。
2.  **AI人格保持の安全性（AI Persona Safety）**
    * ユーザーが「サボっている罪悪感」を感じた瞬間に思考停止は解かれます。
    * AI-DLCによる厳格な批評サイクルを通じ、AIを「正論を吐く秘書」ではなく、ユーザーの保身を全肯定する「共犯者」として完全に固定（アライメント）。精神的な安全性を技術的に保証します。
3.  **返報性ハックの精密タイミング（Algorithm）**
    * 「いつ贈るか」という判断すらユーザーから奪います。
    * 人間関係の「時差」を考慮した二段階返報ロジックを実装。EventBridge Schedulerを用い、社会的評価を最大化する「早すぎず遅すぎない」黄金時間を自律的に計算・実行します。

### 【ビジネスインパクト】「ライフライン」としての依存とCVRの創出

本システムにおいて、ECプラットフォームへの誘導（マネタイズ）は単なる副産物です。しかし、そのCVRは既存の広告モデルを圧倒します。

* **「処方箋」としての商品提示**
    AIが提案する商品は、単なるおすすめギフトではなく、社会的な死を防ぐための **「処方箋（ライフライン）」** として機能します。
* **拒絶不能な購買体験**
    「保身という根源的な動機」と「精密なタイミング」が結合したとき、ユーザーは他商品と比較検討する余地を失い、システムへの完全な依存のもとで購買を完了させます。


AI時代、人間に残る仕事は「共感」や「信頼構築」だと言われています。コミュ♥外chuは、その最後の聖域すらも面倒くさがり、AIに丸投げする人間を育てます。

---

### 🛡️ Engineering Excellence & Quality Control（エンジニアリングの卓越性と品質管理）

### AI-DLCによる反復的批評サイクル

本プロジェクトは、AWS AI-DLC（AI-Driven Lifecycle）の公式ドキュメントに基づき、AIに自身の設計を厳しく「批評（Critique）」させ、人間がそれを監督して修正を繰り返すプロセスを実践しました。
...
**品質保証の成果**：
- 致命的なブロッカー：0件
- Inceptionフェーズの完全性：100%
- Construction Phaseへの移行準備完了

### 実装リスク軽減のための専門ドキュメント

標準的なAI-DLCドキュメントに加え、実装リスクを事前に潰し込むための専門ドキュメントを作成した。

**1. henpou-timing-logic.md**：返報性ハックのタイミング計算ロジック詳細設計
- 社会的マナーに基づく「時差」通知ロジックの完全自動化
- MVP段階（固定21日）と決勝用（動的再計算）の段階的拡張アプローチ
- エッジケース処理の網羅的な定義

**2. ai-persona-prototype.md**：AI人格プロンプトプロトタイプ
- 「ゆるふわカンペAD」の人格要件を厳格に定義
- 人格崩壊パターンの検出・防止メカニズム
- プロンプトバージョン管理とテスト駆動開発（Prompt TDD）

**3. api-contract-specification.md**：API契約仕様書
- フロントエンドとバックエンドのデータ構造の乖離を防ぐ
- 20エンドポイントの詳細なRequest/Responseスキーマ定義

---

## 🏗️ Technical Architecture（アーキテクチャ）

### アーキテクチャの設計思想：「怠慢」のシステム化

本システムは、フルマネージドなサーバーレスアーキテクチャを採用しています。これはインフラ運用のオーバーヘッドを物理的な限界までゼロに近づけるための戦略的決定です。単なる工数削減ではなく、「開発者がコードの保守という低レイヤーの作業から解放され、より高次のビジネスロジックと価値創造へシフトする」というAI-DLCのコア思想を体現しています。コードを書かない、運用しないというエンジニアリングにおける究極の「怠慢」をアーキテクチャレベルで実装しました。
...
- State Management: Zustand + TanStack Query（決勝用）

**Backend**:
- API: Amazon API Gateway
- Compute: AWS Lambda（Python/Node.js）
- Database: Amazon DynamoDB
- Authentication: Amazon Cognito（Google/LINEフェデレーション）
...
```mermaid
graph TD
    User[ユーザー] -->|HTTPS| Amplify[AWS Amplify<br/>React Frontend]
    Amplify -->|REST API| APIGW[Amazon API Gateway]
    
    APIGW -->|認証| Cognito[Amazon Cognito<br/>Google/LINE OAuth]
    APIGW -->|リクエスト| Lambda1[AWS Lambda<br/>API Handler]
    
    Lambda1 -->|CRUD| DynamoDB[(Amazon DynamoDB<br/>Users/Targets/Episodes)]
    Lambda1 -->|AI推論| Bedrock[Amazon Bedrock<br/>Nova Pro]
    Lambda1 -->|スケジュール登録| EventBridge[Amazon EventBridge<br/>Scheduler]
    
    EventBridge -->|24時間後/21日後| Lambda2[AWS Lambda<br/>Notification Handler]
    Lambda2 -->|プッシュ通知| FCM[Firebase Cloud<br/>Messaging]
    FCM -->|通知| User
    
    Calendar[Google Calendar<br/>Webhook] -->|予定変更| Lambda3[AWS Lambda<br/>Calendar Handler]
    Lambda3 -->|スケジュール更新| EventBridge
...
## 🔄 Development Process（開発プロセスとリリース戦略）
...
本プロジェクトに関するお問い合わせは、リポジトリのIssuesまでお願いします。
